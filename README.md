...
We decided to go with a pure Peer-to-peer system. We use our syncronizer aswell as a cyclic counter to maintain the same list of hall orders across all elevators. Each elevator then use the cost calculator, together with the information it has about the other elevators connected to the network, to calculate which orders it should be servicing, and then self assigning these orders.
